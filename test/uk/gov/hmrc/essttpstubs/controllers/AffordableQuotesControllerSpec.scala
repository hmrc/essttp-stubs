...
class AffordableQuotesControllerSpec extends ItSpec {
  ...
  lazy val testAffordableQuotesConnector: TestAffordableQuotesConnector =
    injector.instanceOf[TestAffordableQuotesConnector]
  ...
  "AffordableQuotesController.affordableQuotes should" - {
    "return payment plans with 1, 2 and 3 month options when optimum is 1 or 2" in {
      ...
      result.json.as[AffordableQuotesResponse].paymentPlans.map(_.collections.regularCollections.size) shouldBe List(
        1,
        2,
        3
      )
    ...
    "return payment plans containing initialPayment in collections when there is an upfront payment in request" in {
      ...
      val request: JsValue                              =
        affordableQuotesRequest(totalDebt6k, affordableAmount, Some(UpfrontPaymentAmount(AmountInPence(1000))))
      val result: HttpResponse                          = testAffordableQuotesConnector.calculateAffordableQuotes(request).futureValue
      result.json shouldBe Json.toJson(expectedResult)
    ...
    "allow for interest" in {
      ...
      val request: JsValue                              =
        affordableQuotesRequest(TotalDebt(totalDebt6k.value - interest), affordableAmount, interest = interest)
      val result: HttpResponse                          = testAffordableQuotesConnector.calculateAffordableQuotes(request).futureValue
      result.json shouldBe Json.toJson(expectedResult)
      result.json.as[AffordableQuotesResponse].paymentPlans.map(_.collections.regularCollections.size) shouldBe List(
        1,
    ...
    "not return plan where the collection amount is less than £1" in {
      ...
      val response: AffordableQuotesResponse            =
        testAffordableQuotesConnector.calculateAffordableQuotes(request).futureValue.json.as[AffordableQuotesResponse]
  ...
  def affordableQuotesRequest(
    ...
    val upfrontPaymentJson: String = upfrontPaymentAmount.fold("")(someValue => s"""
