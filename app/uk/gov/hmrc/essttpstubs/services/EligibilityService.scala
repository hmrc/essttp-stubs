...
@Singleton()
class EligibilityService @Inject() (eligibilityRepo: EligibilityRepo)(using ExecutionContext) {
  ...
  def eligibilityData(eligibilityRequest: EligibilityRequest): Future[Option[EligibilityCheckResult]] =
    eligibilityRepo
      .findEligibilityDataByTaxRef(eligibilityRequest.identification)
...
object EligibilityService {
  def defaultEligibleResponse(
    regimeType:         RegimeType,
    identificationList: List[Identification]
  ): EligibilityCheckResult = {
    val identification: List[Identification] = regimeType match {
      case RegimeType.EPAYE =>
        if (identificationList.sizeIs == 1) {
