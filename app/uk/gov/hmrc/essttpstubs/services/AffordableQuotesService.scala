...
class AffordableQuotesService @Inject() (config: Configuration) {
  ...
  def calculateAffordableQuotes(affordableQuotesRequest: AffordableQuotesRequest): AffordableQuotesResponse = {
    val totalDebt: TotalDebt                         = TotalDebt(
      AmountInPence(
        affordableQuotesRequest.debtItemCharges
          .map(x => x.outstandingDebtAmount.value.value)
          .sum + affordableQuotesRequest.accruedDebtInterest.value.value
      )
      ...
      affordableQuotesRequest.initialPaymentAmount
        .flatMap { (someUpfrontPaymentAmount: UpfrontPaymentAmount) =>
          affordableQuotesRequest.initialPaymentDate
            .map(someUpfrontPaymentDate =>
              InitialCollection(DueDate(someUpfrontPaymentDate.value), AmountDue(someUpfrontPaymentAmount.value))
            )
        }
    ...
    val optimumPlan: PaymentPlan                     =
      optimumPaymentPlan(affordableQuotesRequest.paymentPlanAffordableAmount, allPossiblePlans) match {
        case Some(value) => value
        case None        =>
          allPossiblePlans
            .find(p => p.planDuration.value == 1)
            .getOrElse(throw new RuntimeException("There was no optimum plan, investigate..."))
      }
  ...
  private def computeAllPossiblePlans(
    affordableQuotesRequest: AffordableQuotesRequest,
    totalDebt:               TotalDebt,
    interestPerMonth:        AmountInPence,
    initialCollection:       Option[InitialCollection]
  ): List[PaymentPlan] =
    (1 to affordableQuotesRequest.paymentPlanMaxLength.value).map { numberOfInstalments =>
      computeAPlan(
        ...
        totalDebt,
        interestPerMonth,
        affordableQuotesRequest.paymentPlanStartDate,
        initialCollection,
        affordableQuotesRequest
      )
    }.toList

  private def computeAPlan(
    planDuration:            Int,
    totalDebt:               TotalDebt,
    interestPerMonth:        AmountInPence,
    startDate:               InstalmentStartDate,
    initialCollection:       Option[InitialCollection],
    affordableQuotesRequest: AffordableQuotesRequest
  ): PaymentPlan = {
    ...
    PaymentPlan(
      ...
      instalments =
        computeInstalments(affordableQuotesRequest, totalDebt, interestPerMonth, collections.regularCollections.size)
    )
  }

  private def computeCollections(
    planDuration:               Int,
    totalDebtIncludingInterest: TotalDebtIncludingInterest,
    planStartDate:              InstalmentStartDate,
    initialCollection:          Option[InitialCollection]
  ): Collection = {
  ...
  private def optimumPaymentPlan(
    ...
    val collectionsLessThanAffordableAmount: List[PaymentPlan] =
      paymentPlans.filter(
        _.collections.regularCollections.headOption
          .getOrElse(noRegularCollectionsRuntimeError)
          .amountDue
          .value
          .value <= paymentPlanAffordableAmount.value.value
      )
  ...
  private def deriveCollectionsToReturnBasedOnOptimum(
    ...
    if (minDuration < 1)
      Left(
        AffordableQuotesService.CalculationError(
          s"PaymentPlanMinLength was ${minDuration.toString} - expected it to be >= 1"
        )
      )
    else if (maxDuration < 3)
      Left(
        AffordableQuotesService.CalculationError(
          s"PaymentPlanMaxLength was ${maxDuration.toString} - expected it to be >= 3"
        )
      )
    else {
      val result =
        if (optimumDuration <= (minDuration + 1))
          ...
          paymentPlans
            .filter(_.planDuration.value >= (optimumDuration - 1))
            .filter(_.planDuration.value <= (optimumDuration + 1))
  ...
  def computeInstalments(
    affordableQuotesRequest: AffordableQuotesRequest,
    totalDebt:               TotalDebt,
    monthlyInterest:         AmountInPence,
    ...
    def createInstalments(list: List[Instalment], acc: Int): List[Instalment] =
      if (list.sizeIs == numberOfInstalments) list
      else {
        val newInstalment: Instalment = Instalment(
          ...
          debtItemChargeId = affordableQuotesRequest.debtItemCharges
            .map(_.debtItemChargeId)
            .headOption
